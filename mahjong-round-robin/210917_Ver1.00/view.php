...
<a href="preedit.php">成績を更新する・対戦表設定を変更する</a>
...
for($i=0;$i<8;$i++){
	...
	echo (sprintf("%.1f",abs($total_scores[$i]['score'])));
	echo ("pt</td></tr>\n");
}
?>
</table>
...
for($i=0;$i<8;$i++){
		...
		for($k=0;$k<=1;$k++){
			for($l=0;$l<4;$l++){
				if($scores[$j*2+$k][$l]['player'] == $record["player".($i+1)]){
					...
					echo(sprintf("%.1f",abs($scores[$j*2+$k][$l]['f_score']))."pt");
				}else{
					echo(" ");
				}
			}
	...
	for($j=0;$j<8;$j++){
		if($record["player".($i+1)] == $total_scores[$j]['player']){
			...
			echo (sprintf("%.1f",abs($total_scores[$j]['score'])));
		}
	}
	echo ("pt</th></tr>\n");
}
...
for($i=1;$i<=7;$i++){
	...
	for ($j=0;$j<=1;$j++){
		...
		echo ("卓<p>\n");
			...
			echo ("</table>\n");			
			if($urls[($i-1)*2+$j]<>""){
				echo ("<a href=\"".  $urls[($i-1)*2+$j] ."\" target=\"_blank\">[リンク]</a>\n");
			}
